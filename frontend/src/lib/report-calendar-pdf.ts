...
async function loadPdfImage(src: string): Promise<LoadedPdfImage | null> {
...
function drawContainedImage(
  ...
  context.drawImage(image.image, drawX, drawY, width, height);
}

function drawHeaderLogoBelowCard(
  context: CanvasRenderingContext2D,
  companyLogo: LoadedPdfImage | null,
  metricCardX: number,
  metricCardY: number,
  metricCardWidth: number,
  metricCardHeight: number,
) {
  if (!companyLogo) {
    return;
  }
  ...
  drawContainedImage(
    ...
    companyLogo,
    metricCardX,
    metricCardY + metricCardHeight + 8,
    metricCardWidth,
    56,
  );
}
...
function drawPdfPageFooter(
  ...
  const footerY = PAGE_PIXEL_HEIGHT - 18;
  ...
  context.fillText("Generated by Devant Solutions Timesheet Software", PAGE_PADDING, footerY);

  context.textAlign = "right";
  context.fillText(`Page ${pageIndex + 1} of ${pageCount}`, PAGE_PIXEL_WIDTH - PAGE_PADDING, footerY);
}
...
function renderWeeklyCalendarPage(
  ...
  drawHeaderLogoBelowCard(context, companyLogo, metricCardX, metricCardY, metricCardWidth, metricCardHeight);
  ...
  drawPdfPageFooter(context, pageIndex, pageCount);
...
export async function buildMonthlyCalendarPdf(options: MonthlyCalendarPdfOptions) {
  ...
  const companyLogo = await loadPdfImage("/plannit-logo.svg");

  for (const [pageIndex, page] of weekPages.entries()) {
    const canvas = renderWeeklyCalendarPage(page, options, companyLogo, pageIndex, totalPageCount);
